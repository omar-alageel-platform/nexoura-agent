...
Per T13 §7: report-class deliverables — NFR register, SLO/SLI matrix, contradiction log, Stage 2 NFR-readiness memo — emit as both `.md` source-of-truth and `.docx` generated by pandoc, committed alongside in the same PR. `.md` canonical; `.docx` for client delivery. Internal verification notes are `.md` only.

SOUL.md itself is `.md` only.
